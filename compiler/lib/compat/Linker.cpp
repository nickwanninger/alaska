...
namespace {
  ...
  class ModuleLinker {
    ...
    bool shouldLinkFromSource(bool &LinkFromSrc, const GlobalValue &Dest, const GlobalValue &Src);
    ...
    bool getComdatLeader(Module &M, StringRef ComdatName, const GlobalVariable *&GVar);
    bool computeResultingSelectionKind(StringRef ComdatName, Comdat::SelectionKind Src,
        Comdat::SelectionKind Dst, Comdat::SelectionKind &Result, LinkFrom &From);
    DenseMap<const Comdat *, std::pair<Comdat::SelectionKind, LinkFrom>> ComdatsChosen;
    bool getComdatResult(const Comdat *SrcC, Comdat::SelectionKind &SK, LinkFrom &From);
    ...
    GlobalValue *getLinkedToGlobal(const GlobalValue *SrcGV) {
      ...
      if (!SrcGV->hasName() || GlobalValue::isLocalLinkage(SrcGV->getLinkage())) return nullptr;
    ...
    void dropReplacedComdat(GlobalValue &GV, const DenseSet<const Comdat *> &ReplacedDstComdats);
...
static GlobalValue::VisibilityTypes getMinVisibility(
  ...
  if (A == GlobalValue::ProtectedVisibility || B == GlobalValue::ProtectedVisibility)
    return GlobalValue::ProtectedVisibility;
  return GlobalValue::DefaultVisibility;
}

bool ModuleLinker::getComdatLeader(Module &M, StringRef ComdatName, const GlobalVariable *&GVar) {
...
bool ModuleLinker::computeResultingSelectionKind(StringRef ComdatName, Comdat::SelectionKind Src,
    Comdat::SelectionKind Dst, Comdat::SelectionKind &Result, LinkFrom &From) {
  Module &DstM = Mover.getModule();
  // The ability to mix Comdat::SelectionKind::Any with
  // Comdat::SelectionKind::Largest is a behavior that comes from COFF.
  bool DstAnyOrLargest = Dst == Comdat::SelectionKind::Any || Dst == Comdat::SelectionKind::Largest;
  bool SrcAnyOrLargest = Src == Comdat::SelectionKind::Any || Src == Comdat::SelectionKind::Largest;
  if (DstAnyOrLargest && SrcAnyOrLargest) {
    if (Dst == Comdat::SelectionKind::Largest || Src == Comdat::SelectionKind::Largest)
    ...
    return emitError("Linking COMDATs named '" + ComdatName + "': invalid selection kinds!");
  }
  ...
  switch (Result) {
    ...
    case Comdat::SelectionKind::SameSize: {
      ...
      if (getComdatLeader(DstM, ComdatName, DstGV) || getComdatLeader(*SrcM, ComdatName, SrcGV))
        return true;
      ...
      if (Result == Comdat::SelectionKind::ExactMatch) {
        if (SrcGV->getInitializer() != DstGV->getInitializer())
          return emitError("Linking COMDATs named '" + ComdatName + "': ExactMatch violated!");
        From = LinkFrom::Dst;
      } else if (Result == Comdat::SelectionKind::Largest) {
        From = SrcSize > DstSize ? LinkFrom::Src : LinkFrom::Dst;
      } else if (Result == Comdat::SelectionKind::SameSize) {
        if (SrcSize != DstSize)
          return emitError("Linking COMDATs named '" + ComdatName + "': SameSize violated!");
        From = LinkFrom::Dst;
      } else {
...
bool ModuleLinker::getComdatResult(
    const Comdat *SrcC, Comdat::SelectionKind &Result, LinkFrom &From) {
  Module &DstM = Mover.getModule();
...
bool ModuleLinker::shouldLinkFromSource(
    bool &LinkFromSrc, const GlobalValue &Dest, const GlobalValue &Src) {
  ...
  return emitError("Linking globals named '" + Src.getName() + "': symbol multiply defined!");
}

bool ModuleLinker::linkIfNeeded(GlobalValue &GV, SmallVectorImpl<GlobalValue *> &GVToClone) {
  ...
  if (DGV && !GV.hasLocalLinkage() && !GV.hasAppendingLinkage()) {
    ...
    if (DGVar && SGVar) {
      ...
      if (DGVar->hasCommonLinkage() && SGVar->hasCommonLinkage()) {
        MaybeAlign Align(std::max(DGVar->getAlignment(), SGVar->getAlignment()));
    ...
    GlobalValue::UnnamedAddr UnnamedAddr =
        GlobalValue::getMinUnnamedAddr(DGV->getUnnamedAddr(), GV.getUnnamedAddr());
  ...
  if (const Comdat *SC = GV.getComdat()) {
    ...
    if (ComdatFrom == LinkFrom::Dst) return false;
  }

  bool LinkFromSrc = true;
  if (DGV && shouldLinkFromSource(LinkFromSrc, *DGV, GV)) return true;
  if (DGV && ComdatFrom == LinkFrom::Both) GVToClone.push_back(LinkFromSrc ? DGV : &GV);
...
void ModuleLinker::addLazyFor(GlobalValue &GV, const IRMover::ValueAdder &Add) {
  ...
  if (!GV.hasLinkOnceLinkage() && !GV.hasAvailableExternallyLinkage() && !shouldLinkOnlyNeeded())
    return;
...
void ModuleLinker::dropReplacedComdat(
  ...
  if (auto *F = dyn_cast<Function>(&GV)) {
    ...
    if (auto *FTy = dyn_cast<FunctionType>(Alias.getValueType())) {
      ...
      Declaration = new GlobalVariable(M, Alias.getValueType(), /*isConstant*/ false,
...
bool ModuleLinker::run() {
  ...
  for (const auto &SMEC : SrcM->getComdatSymbolTable()) {
    ...
    LinkFrom From;
    if (getComdatResult(&C, SK, From)) return true;
    ComdatsChosen[&C] = std::make_pair(SK, From);

    if (From != LinkFrom::Src) continue;
  ...
  for (GlobalVariable &GV : SrcM->globals())
    if (GV.hasLinkOnceLinkage())
      if (const Comdat *SC = GV.getComdat()) LazyComdatMembers[SC].push_back(&GV);

  for (Function &SF : *SrcM)
    if (SF.hasLinkOnceLinkage())
      if (const Comdat *SC = SF.getComdat()) LazyComdatMembers[SC].push_back(&SF);

  for (GlobalAlias &GA : SrcM->aliases())
    if (GA.hasLinkOnceLinkage())
      if (const Comdat *SC = GA.getComdat()) LazyComdatMembers[SC].push_back(&GA);

  // Insert all of the globals in src into the DstM module... without linking
  // initializers (which could refer to functions not yet mapped over).
  SmallVector<GlobalValue *, 0> GVToClone;
  for (GlobalVariable &GV : SrcM->globals())
    if (linkIfNeeded(GV, GVToClone)) return true;

  for (Function &SF : *SrcM)
    if (linkIfNeeded(SF, GVToClone)) return true;

  for (GlobalAlias &GA : SrcM->aliases())
    if (linkIfNeeded(GA, GVToClone)) return true;

  for (GlobalIFunc &GI : SrcM->ifuncs())
    if (linkIfNeeded(GI, GVToClone)) return true;
  ...
  for (GlobalValue *GV : GVToClone) {
    if (auto *Var = dyn_cast<GlobalVariable>(GV)) {
      auto *NewVar = new GlobalVariable(*Var->getParent(), Var->getValueType(), Var->isConstant(),
          Var->getLinkage(), Var->getInitializer());
      ...
      if (Var->getParent() != &Mover.getModule()) ValuesToLink.insert(NewVar);
    } else {
      emitError(
          "linking '" + GV->getName() + "': non-variables in comdat nodeduplicate are not handled");
    }
  }
  ...
  if (Error E = Mover.move(std::move(SrcM), ValuesToLink.getArrayRef(),
          IRMover::LazyCallback([this](GlobalValue &GV, IRMover::ValueAdder Add) {
            addLazyFor(GV, Add);
          }),
...
bool Linker::linkInModule(std::unique_ptr<Module> Src, unsigned Flags,
    std::function<void(Module &, const StringSet<> &)> InternalizeCallback) {
  ModuleLinker ModLinker(Mover, std::move(Src), Flags, std::move(InternalizeCallback));
