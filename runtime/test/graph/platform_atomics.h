...
bool compare_and_swap(float &x, const float &old_val, const float &new_val) {
  return __sync_bool_compare_and_swap(reinterpret_cast<uint32_t *>(&x),
      reinterpret_cast<const uint32_t &>(old_val), reinterpret_cast<const uint32_t &>(new_val));
}
...
bool compare_and_swap(double &x, const double &old_val, const double &new_val) {
  return __sync_bool_compare_and_swap(reinterpret_cast<uint64_t *>(&x),
      reinterpret_cast<const uint64_t &>(old_val), reinterpret_cast<const uint64_t &>(new_val));
}
...
bool compare_and_swap(float &x, const float &old_val, const float &new_val) {
  return old_val == atomic_cas_32((volatile uint32_t *)&x, (const volatile uint32_t &)old_val,
                        (const volatile uint32_t &)new_val);
}

bool compare_and_swap(double &x, const double &old_val, const double &new_val) {
  return old_val == atomic_cas_64((volatile uint64_t *)&x, (const volatile uint64_t &)old_val,
