...
class UniDist {
 public:
  UniDist(NodeID_ max_value, rng_t_ &rng)
      : rng_(rng) {
...
template <typename NodeID_, typename DestID_ = NodeID_, typename WeightT_ = NodeID_,
    ...
    typename rng_t_ =
        typename std::conditional<(uNodeID_bits_ == 32), std::mt19937, std::mt19937_64>::type>
class Generator {
...
#pragma omp parallel for
  ...
  EdgeList &&preprocess(EdgeList &&el) {
