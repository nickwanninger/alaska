...
namespace ck {
  ...
  class basic_string_view {
   ...
   public:
    ...
    basic_string_view(const T* start, size_t len)
        : m_start(start)
        , m_len(len) {
    }

    basic_string_view(T* start)
        : m_start(start) {
      for (m_len = 0; start[m_len] != 0; m_len++) {
      }
    }

    basic_string_view(const T* start)
        : m_start(start) {
      for (m_len = 0; start[m_len] != 0; m_len++) {
      }
    }
...
#define round_up(x, y) (((x) + (y)-1) & ~((y)-1))
    void reserve(unsigned int new_cap) {
      ...
      if (m_buf == nullptr) {
        ALASKA_ASSERT(m_cap == 0, "Cap must be 0 when buf is null.");
        m_buf = (T*)malloc(new_cap * sizeof(T));
        memset(m_buf, 0, new_cap);

        ALASKA_ASSERT(m_buf != nullptr, "buf must not be null");
      } else {
