...
public:
  ...
  StackMapParser(uint8_t *StackMapSection)
    ...
    for (unsigned I = 0, E = getNumRecords(); I != E; ++I) {
      StackMapRecordOffsets.push(CurrentRecordOffset);
      CurrentRecordOffset +=
  ...
  FunctionAccessor getFunction(unsigned FunctionIndex) const {
    return FunctionAccessor(StackMapSection +
  ...
  function_iterator functions_end() const {
    return function_iterator(
             FunctionAccessor(StackMapSection +
...
private:
  ...
  ck::vec<unsigned> StackMapRecordOffsets;
};

} // end namespace alaska
