...
attribute_hidden void splay_tree_insert(splay_tree sp, splay_tree_node node) {
  int comparison = 0;

  splay_tree_splay(sp, &node->key);

  if (sp->root) comparison = splay_compare(&sp->root->key, &node->key);
...
attribute_hidden void splay_tree_remove(splay_tree sp, splay_tree_key key) {
...
attribute_hidden splay_tree_key splay_tree_lookup(splay_tree sp, splay_tree_key key) {
  splay_tree_splay(sp, key);
...
static void splay_tree_foreach_internal(splay_tree_node node, splay_tree_callback func, void *data) {
  if (!node) return;
  func(&node->key, data);
...
attribute_hidden void splay_tree_foreach(splay_tree sp, splay_tree_callback func, void *data) {
