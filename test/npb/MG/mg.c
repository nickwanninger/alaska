...
#pragma omp master
  ...
  c_print_results("MG", Class, nx[lt], ny[lt], nz[lt], nit, nthreads, t, mflops, "          floating point", verified,
...
static void setup(int *n1, int *n2, int *n3, int lt) {
  ...
  *n3 = nz[lt] + 2;
  ...
  if (debug_vec[1] >= 1) {
    ...
    printf("%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d\n", lt, nx[lt], ny[lt], nz[lt], *n1, *n2, *n3, is1, is2, is3, ie1,
        ie2, ie3);
  }
}
...
#pragma omp parallel for default(shared) private(i1, i2, i3, r1, r2)
  for (i3 = 1; i3 < n3 - 1; i3++) {
    for (i2 = 1; i2 < n2 - 1; i2++) {
      ...
      for (i1 = 1; i1 < n1 - 1; i1++) {
        u[i3][i2][i1] = u[i3][i2][i1] + c[0] * r[i3][i2][i1] + c[1] * (r[i3][i2][i1 - 1] + r[i3][i2][i1 + 1] + r1[i1]) +
        ...
        for (i2 = 1; i2 <= mm2 - 1; i2++) {
          ...
          for (i1 = 1; i1 <= mm1 - 1; i1++) {
            u[2 * i3 - t3 - 1][2 * i2 - t2 - 1][2 * i1 - t1 - 1] =
                u[2 * i3 - t3 - 1][2 * i2 - t2 - 1][2 * i1 - t1 - 1] +
                0.125 *
                    (z[i3][i2][i1] + z[i3][i2 - 1][i1] + z[i3][i2][i1 - 1] + z[i3][i2 - 1][i1 - 1] + z[i3 - 1][i2][i1] +
                        z[i3 - 1][i2 - 1][i1] + z[i3 - 1][i2][i1 - 1] + z[i3 - 1][i2 - 1][i1 - 1]);
          }
        }
      }
    }
